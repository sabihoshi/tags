...
{while;{logic;!;{regextest;{get;~f};/^(?:-?(?:\d+\.\d*|\.?\d+)$|NaN)/}};
	...
	{exec;whileregexinject;"~f" "/\((-?(?:\d+\.\d*|\.?\d+))\)([-+/*])(-?(?:\d+\.\d*|\.?\d+))|(-?(?:\d+\.\d*|\.?\d+))([-+/*])\((-?(?:\d+\.\d*|\.?\d+))\)/" "{lb}lb{rb}math{lb}semi{rb}$2$5{lb}semi{rb}$1$4{lb}semi{rb}$3$5{lb}rb{rb}"}{//;Solve (x)+y, (x)*y, (x)/y, (x)-y}{if;{get;~v};{if;{get;~nf};!=;{get;~f};{set;~nf;{get;~f}}15 - {get;~f}}}
